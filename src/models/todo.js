...
const create = async (data, user) => {
  if (!_.hasIn(data, 'task') || !_.isString(data.task))
    throw new Error('data.task needs to be set as string');
...
const update = async (id, data, user) => {
  if (!_.isString(id)) throw new Error('id needs to be a string');
  if (_.hasIn(data, 'task') && !_.isString(data.task))
    throw new Error('data.task should be a string');
  if (_.hasIn(data, 'completed') && !_.isBoolean(data.completed))
    throw new Error('data.completed should to be a boolean');
